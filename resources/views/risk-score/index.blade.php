...
@section('title', 'Risk Score & Early Warning')
@section('subtitle', 'Analisis risiko collection & churn pelanggan berdasarkan scoring rule-based C3MR')
    ...
    <div class="col-6 col-lg-3">
        <div class="card kpi-card h-100" style="border-left: 4px solid var(--danger);">
            <div>
                ...
                <div class="kpi-value" style="color:var(--danger);">{{ number_format($riskCounts['CRITICAL'] ?? 0) }}</div>
    ...
    <div class="col-6 col-lg-3">
        <div class="card kpi-card h-100" style="border-left: 4px solid var(--warning);">
            <div>
                ...
                <div class="kpi-value" style="color:var(--warning);">{{ number_format($riskCounts['HIGH'] ?? 0) }}</div>
    ...
    <div class="col-6 col-lg-3">
        <div class="card kpi-card h-100" style="border-left: 4px solid #CA8A04;">
            <div>
                ...
                <div class="kpi-value" style="color:#A16207;">{{ number_format($riskCounts['MEDIUM'] ?? 0) }}</div>
    ...
    <div class="col-6 col-lg-3">
        <div class="card kpi-card h-100" style="border-left: 4px solid var(--success);">
            <div>
                ...
                <div class="kpi-value" style="color:var(--success);">{{ number_format($riskCounts['LOW'] ?? 0) }}</div>
...
<div class="filter-bar mb-3">
    <form method="GET" action="{{ url('/risk-score') }}" class="d-flex w-100 gap-2 flex-wrap align-items-end">
        ...
        <div class="d-flex gap-2">
            ...
            @if($search || $filterLevel)
                <a href="{{ url('/risk-score') }}" class="btn btn-outline-secondary btn-sm" style="border-radius:8px; height:31px;">Reset</a>
            @endif
            <a href="{{ route('risk-score.export', request()->query()) }}" class="btn btn-sm" style="background:var(--success-soft); color:var(--success); border:1px solid rgba(22,163,74,0.3); font-weight:600; border-radius:8px; height:31px; display:inline-flex; align-items:center; gap:5px;">
...
<div class="card p-0" style="overflow:hidden;">
    <div style="padding:16px 20px; border-bottom:1px solid var(--border); display:flex; justify-content:space-between; align-items:center;">
        <div>
            ...
            <div style="font-size:11.5px; color:var(--ink-400);">{{ $customers->total() }} pelanggan terdaftar</div>
        </div>
    </div>
            ...
            <tbody>
                @forelse($customers as $idx => $customer)
                    <tr>
                            ...
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <div>
                                    ...
                                    <div style="font-size:11px; color:var(--ink-400);">
                                        {{ $customer->nama_layanan_internet ?? 'Internet' }}
                                    </div>
                                </div>
                            </div>
                        </td>
                        <td>
                            <div class="d-flex align-items-center gap-1 masked-snd-wrapper" data-snd="{{ $customer->nomor_internet }}" data-masked="true">
                                ...
                                <button type="button" class="btn btn-link p-0 text-muted" style="font-size:11px; line-height:1; opacity:0.6;" onclick="copyToClipboard('{{ $customer->nomor_internet }}', this)" title="Salin No Internet" data-bs-toggle="tooltip">
                        ...
                        <td style="text-align:right; font-weight:700; color:{{ $customer->saldo_piutang > 0 ? 'var(--danger)' : 'var(--ink-400)' }}; font-size:13px; white-space:nowrap;">
                        ...
                        <td style="text-align:center; font-weight:800; font-size:14px; color:var(--ink-900);">
                            {{ $customer->risk_score_calc }}
                        </td>
                        <td style="text-align:center;">
                            @if($customer->risk_level_calc === 'CRITICAL')
                                <span class="badge-status badge-risk-critical">CRITICAL</span>
                            @elseif($customer->risk_level_calc === 'HIGH')
                                <span class="badge-status badge-risk-high">HIGH</span>
                            @elseif($customer->risk_level_calc === 'MEDIUM')
                        ...
                        <td>
                            ...
                            @if(!empty($customer->risk_reasons))
                                <div style="font-size:11px; color:var(--ink-400); margin-top:2px;">
                                    {{ implode(', ', array_slice($customer->risk_reasons, 0, 2)) }}
                                </div>
                            @endif
                        </td>
                        <td style="text-align:center;">
                            <a href="{{ url('/customers/' . $customer->id) }}" class="btn btn-sm btn-outline-telkom" style="font-size:11.5px; padding:3px 8px; white-space:nowrap;" title="Lihat detail risiko pelanggan" data-bs-toggle="tooltip">

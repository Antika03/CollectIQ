...
<div id="syncResultContainer" class="card mb-4" style="{{ ($lastSyncResult || session('syncResult')) ? '' : 'display:none;' }}">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3 pb-2" style="border-bottom:1px solid var(--border);">
        ...
        <div>
            <span class="badge" id="resultSummaryBadge" style="background:var(--secondary); color:var(--ink-700); font-weight:600; font-size:11.5px; padding:6px 12px; border-radius:8px;">
                <span id="resultProcessedCount">{{ number_format($activeResult['total_rows_processed'] ?? ($totalCustomers + $totalVisits + $totalCaring)) }}</span> records diproses
            </span>
        </div>
    </div>

    {{-- DATA QUALITY & CONSISTENCY SUMMARY CARD --}}
    <div class="p-3 mb-3" style="background:#F8FAFC; border:1px solid #E2E8F0; border-radius:12px;" id="dataQualityBox">
        <div class="d-flex justify-content-between align-items-center mb-2">
            ...
            <span class="badge {{ ($dq['is_consistent'] ?? true) ? 'bg-success' : 'bg-warning text-dark' }}" id="dqConsistencyBadge" style="font-size:11px; padding:4px 8px; border-radius:6px;">
                <i class="bi bi-shield-check"></i> <span id="dqConsistencyText">{{ ($dq['is_consistent'] ?? true) ? 'Data Konsisten (100%)' : 'Perlu Sinkronisasi Penuh' }}</span>
            </span>
        </div>
        <div class="row g-2">
            <div class="col-4 col-md-2">
                <div class="dq-item">
                    <div class="dq-value" id="dqSourceRows">{{ number_format($dq['source_sheet_rows'] ?? 27547) }}</div>
                    <div class="dq-label">Baris Sheet</div>
                </div>
            </div>
            <div class="col-4 col-md-2">
                <div class="dq-item">
                    <div class="dq-value text-success" id="dqProcessedRows">{{ number_format($details['data_all']['count'] ?? $totalCustomers) }}</div>
                    <div class="dq-label">Valid Diproses</div>
                </div>
            </div>
            <div class="col-4 col-md-2">
                <div class="dq-item">
                    <div class="dq-value text-primary" id="dqCreated">{{ number_format($dq['created_customers'] ?? 0) }}</div>
                    <div class="dq-label">Pelanggan Baru</div>
                </div>
            </div>
            <div class="col-4 col-md-2">
                <div class="dq-item">
                    <div class="dq-value text-info" id="dqUpdated">{{ number_format($dq['updated_customers'] ?? $totalCustomers) }}</div>
                    <div class="dq-label">Diperbarui</div>
                </div>
            </div>
            <div class="col-4 col-md-2">
                <div class="dq-item">
                    <div class="dq-value text-warning" id="dqDuplicates">{{ number_format($dq['duplicate_in_source'] ?? 1021) }}</div>
                    <div class="dq-label">Duplikat Sheet</div>
                </div>
            </div>
            <div class="col-4 col-md-2">
                <div class="dq-item">
                    <div class="dq-value text-success" id="dqValidPhones">{{ number_format($validPhones) }}</div>
        ...
        <div class="card kpi-card h-100">
            <div>
                ...
                <div style="font-size:11px; color:var(--success); margin-top:4px;">
                    <i class="bi bi-check-circle-fill"></i> <span id="kpiPhones">{{ number_format($validPhones) }}</span> No HP valid
                </div>
            </div>
